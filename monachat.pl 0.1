	...
	if   ( $command =~ /\/login/ )
	 ...
	 if ( $1 =~ /here\s*(.*)\s*(.*)/ )
	   ...
	   sleep($seconds);
	    ...
	    { system("start socket.pl -proxy -timeout 1"); }
	 }
	...
	$roomdata{$room} = $number;
	 ...
	 while( $read =~ /<USER r="(\d{1,3})" name="(.*?)" id="(\d{1,3})"(.+?)ihash="(.{10})" \w{4,6}="(.*?)" g="(\d{1,3})" type="(.*?)" b="(\d{1,3})" y="(.*?)" x="(.*?)" scl="(.+?)" \/>/g )
	  ...
	  if ( $trip =~ /trip="(.+?)"/ )
	     ...
	     { $trip = " "; }
	  if ( !$roomdata{$keys} )
	     { $roomdata{$keys} = "$name$trip$ihash"; }
	  else
	     { $roomdata{$keys} = $roomdata{$keys} . ", " . "$name$trip$ihash"; }
	  }
	 }
	   }
	 }
...
$pingthread       = threads->create(\&ping, "20") or die "There has been a problem with ping thread.\n";
$readsocketthread = threads->create(\&readsocket) or die "There has been a problem with readsocket thread.\n";

Win32::GUI->Dialog() or die "Can't open main window.\n";
