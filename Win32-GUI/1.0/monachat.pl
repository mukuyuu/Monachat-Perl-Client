...
### - Annoying sound when pressing enter in inputfield
...
share($LANGUAGE);
share($ENCODING);
share(@popuptrigger); ### Has to be declared before being populated
...
$ENABLE_GRAPHIC_INTERFACE = 1 if $config{graphicinterface};
($LANGUAGE, $ENCODING)    = $config{language} eq "english" ? ("english", "cp932") : ("japanese", "UTF-8");
@popuptrigger = split(/ : /, $config{trigger});
chomp(@popuptrigger); ### Is it neccesary?
...
sub command_handler
    ...
    ###
         ...
         {
         my $argument = $1;
         my $command  = $1 if $argument =~ /"(.+?)"/;
         my $timeout  = $1 if $argument =~ /timeout (\d+)/;
         my $wait     = $1 if $argument =~ /wait (\d+)/;
         my $number   = $1 if $argument =~ /number (\d+)/;
         
         print "command: $command, timeout: $timeout, wait: $wait, number: $number.\n";
         return if !$command;
...
sub new_instance
    ...
    my $x         = $config{xrandom}   ? int(rand(600) + 1) : "";
    my $y         = $config{yrandom}   ? int(rand(400) + 1) : 275;
    my $scl       = $config{sclrandom} ? int((rand(10) > 5 ? 100 : -100) + 1) : 100;
    ...
    $loginuserdata .= qq{ -room $room -name "$name" -character "$character" -status "$status"} if $search =~ /here\s*(.*)/;
...
sub refresh_main
    ...
    if( !$room2 or $room2 eq "main")
      ...
      $outputfield->ReplaceSel("");
      print_output("ROOM", "Monachat");
    
      ### Format screen
      foreach my $room (sort(keys %mainroom))
          {
      ...
      print_data("USER", $_) foreach (sort(keys %roomid));
      }
    }
...
sub trip_store
    {
    my $tripfile = $NEWINSTANCE ? "tripbackup//newinstance".int(rand(100_000_000_000_000_000)).".txt" : "trip.txt";
    ...
    while()
         ...
         if   ( !-e $tripfile )
              ...
              open(TRIP, "<", $tripfile) or print_output("ERROR", TRIP_ERROR) and redo;
              my @trip = <TRIP>;
...
sub get_name
    ...
    ### Load file
    ### TODO: Print directly on aliased handler
    open(TRIP, "<", "trip.txt") or print_output("ERROR", TRIP_ERROR) and return;
    @tripfile = <TRIP>;
    chomp(@tripfile);
    close(TRIP);
    ...
    foreach $trip (@triplist)
        ...
        print_output("ERROR", TRIP_NOT_FOUND) if !$match;
        $match = 0;
        }
    }
...
sub read_handler
    ...
    while( @read )
         ...
         if   ( $read[0] =~ /\+connect id=\d{1,3}/ ) {}
              ...
              #<COM cmt="(.+?)" cnt="(.+?)" id="(.+?)" \/>
              #<COM cmt="(.+?)" id="(.+?)" \/>
              #<SET cmd="com" pre="" param="(.+?)" id=(\d{1,3}) \/>
              my($comment, $id) = ($1, $2);
              my $name    = $userdata->get_name($id);
...
#share(@popuptrigger);      ### List of strings that trigger an allarm, has to be declared at config load time
