...
sub command_handler
    ...
    elsif( $command =~ /\/random\s*(.*)\s*(.*)/ )
         {
         my $sex     = $1 ? $1 : rand(10) > 5 ? "male" : "female";
         #my $rand   = rand(10);
         my $country = $2 ? $2 : "japan";
         
         print "Getting name (sex: $sex, country: $country)...";
         $userdata->random($loginid, $sex, $country);
         $writesocketqueue->enqueue("REENTER");
         print "done.\n";
         }
...
sub refresh_main
    ...
    if( $room2 and $room2 != 0 and $room2 ne "main")
      {
...
sub read_handler
    ...
    while( @read )
         ...
         if   ( $read[0] =~ /\+connect id=\d{1,3}/ ) {}
              ...
              my $enterroom = ENTER_ROOM_ID;
              
              my $nameihash = $name =~ /^\s+$/ ? $ihash : "$name $ihash";
              $enterroom   .= " (Maybe it's $nameihash ?)" if $ihash;
