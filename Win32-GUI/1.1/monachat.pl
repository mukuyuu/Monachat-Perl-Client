...
sub get_name
    ...
    if   ( $search =~ /^(.{10})$/ )   { $trip = $1; }
    elsif( $search =~ /^(\d{1,3})$/ ) { $trip = $userdata->get_ihash($1); }
    else { return; }
...
sub get_trip
    ...
    elsif( $search =~ /^(\d{1,3})$/ ) { $searchname = encode("utf8", $userdata->get_name($1)); }
         ...
         if   ( $read[0] =~ /\+connect id=\d{1,3}/ ) {}
              ...
              if( $option{popup} )
                ...
                if   ($option{popupall})
                     ...
                     foreach my $trigger (@{$config{trigger}})
                         {
