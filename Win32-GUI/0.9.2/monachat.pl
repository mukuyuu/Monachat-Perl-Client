...
### - /change doesn't work
### - Too many lines when printing log
...
### Load libraries
### There are variables loaded from main from userdata, and therefore has to be loaded after configuration
use lib "lib";
use Userdata;
use Language;
...
sub OptionListView_ItemClick
    ...
    $index == 16 ? option_menu_savelog()     :
    $index == 17 ? option_menu_relogin()     :
    $index == 18 ? option_menu_disconnect()  :
    undef;
    }
...
sub new_instance
    ...
    if( $search =~ /here\s*(.*)/ )
      ...
      for(1..$instancecounter)
         {
         system("start perl monachat.pl -proxy yes -room $room -x $x -y $y -scl $scl -site $site -new");
         sleep(1);
         }
      }
    else { system("start perl monachat.pl -proxy yes -x $x -y $y -scl $scl -site $site -new"); }
    }
...
$optionlistview->InsertItem( -text => "Save log" );         #16
$optionlistview->InsertItem( -text => "Relogin" );          #17
$optionlistview->InsertItem( -text => "Disconnect" );       #18
